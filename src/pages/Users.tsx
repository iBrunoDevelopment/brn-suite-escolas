...
const Users: React.FC<UsersProps> = ({ user }) => {
    ...
    const fetchRolePermissions = async () => {
        ...
        try {
            ...
            // Non-blocking alert for devs/admin
            ...
            {activeTab === 'users' ? (
                ...
                <div className="card overflow-hidden">
                    <div className="p-6 border-b border-white/5 bg-primary/5">
                        ...
                        <p className="text-xs text-slate-500 mt-1">Estes usuários se cadastraram mas ainda não possuem acesso completo ao sistema.</p>
                    </div>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {pendingApprovals.map(u => (
                                    <tr key={u.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button onClick={() => handleEdit(u)} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-black rounded-lg transition-all shadow-lg shadow-emerald-500/20 flex items-center gap-2 ml-auto">
                ...
                <div className="card overflow-hidden">
                    <div className="p-6 border-b border-white/5 bg-blue-500/5">
                        ...
                        <p className="text-xs text-slate-500 mt-1">Mensagens enviadas pelos usuários através do portal.</p>
                    </div>
                    {supportRequests.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 divide-y divide-slate-100 dark:divide-slate-700">
                            {supportRequests.map(req => (
                                <div key={req.id} className="p-6 hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-all flex flex-col md:flex-row gap-6">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${req.status === 'Pendente' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
                                                req.status === 'Em Atendimento' ? 'bg-blue-500/10 text-blue-500 border border-blue-500/20' :
                                        ...
                                        <h4 className="font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                            {req.name}
                                            <span className="text-xs font-normal text-slate-400">({req.email})</span>
                                        </h4>
                                        <p className="text-sm text-slate-600 dark:text-slate-300 mt-2 bg-slate-100/50 dark:bg-slate-900/50 p-4 rounded-2xl italic border border-slate-200/50 dark:border-slate-700/50">
                                            "{req.message}"
                                        </p>
                                    ...
                                    <div className="flex shrink-0 gap-2 items-start md:mt-6">
                                        {req.status !== 'Resolvido' && (
                                            <>
                                                <button
                                                    onClick={() => handleSupportStatus(req.id, 'Em Atendimento')}
                                                    className="px-3 py-1.5 bg-blue-500/10 hover:bg-blue-500/20 text-blue-500 text-[10px] font-black rounded border border-blue-500/30 uppercase transition-all"
                                                >
                                                    ATENDER
                                                </button>
                                                <button
                                                    onClick={() => handleSupportStatus(req.id, 'Resolvido')}
                                                    className="px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-600 hover:text-white text-emerald-500 text-[10px] font-black rounded border border-emerald-500/30 uppercase transition-all"
                                                >
                                                    RESOLVER
                                                </button>
                                            </>
                ...
                <div className="card max-w-2xl mx-auto overflow-hidden animate-in fade-in slide-in-from-bottom-4">
                    <div className="p-8 border-b border-white/5 bg-primary/5">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-white uppercase tracking-wider">Enviar Comunicado Geral</h3>
                    ...
                    <div className="p-8 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                type="text"
                                placeholder="Ex: Manutenção agendada ou Prazo de entrega"
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea
                                rows={6}
                                placeholder="Escreva aqui os detalhes do aviso aos diretores e funcionários..."
                                value={announcement.message}
                                onChange={e => setAnnouncement({ ...announcement, message: e.target.value })}
                                className="input-field py-4 min-h-[150px]"
                            />
                        </div>
                        ...
                        <div className="pt-4 flex justify-end">
                            <button
                                ...
                                className="bg-primary hover:bg-primary-hover text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-primary/20 flex items-center gap-3 transition-all active:scale-95 disabled:opacity-50"
                            >
                                {sendingNotice ? (
                                    <>
                    ...
                    <div className="p-6 bg-black/20 border-t border-white/5 flex items-center gap-4">
                        ...
                        <p className="text-[10px] text-slate-500 font-bold uppercase leading-relaxed">
                            Atenção: Esta ação é irreversível e enviará uma notificação em tempo real para todos os dispositivos conectados.
                        </p>
                    </div>
                </div>
            ) : activeTab === 'closure' ? (
                    ...
                    <div className="card pb-8 overflow-hidden">
                        <div className="p-8 border-b border-white/5 bg-amber-500/5">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-black text-white uppercase tracking-wider">Encerramento de Exercício</h3>
                                ...
                                <div className="flex items-center gap-3">
                                    <input
                                        type="text"
                                        placeholder="Nome do Novo Período (ex: 2026/1)"
                                        value={closurePeriodName}
                                        onChange={e => setClosurePeriodName(e.target.value)}
                                        className="input-field w-64 bg-slate-900"
                                    />
                                    <button
                                        onClick={handleExecuteClosure}
                                        disabled={isCalculatingClosure || closureSummary.length === 0}
                        ...
                        {isCalculatingClosure ? (
                            <div className="p-20 text-center">
                                ...
                                <p className="text-sm text-slate-500 mt-4">Calculando saldos reprogramáveis...</p>
                            </div>
                        ) : closureSummary.length === 0 ? (
                            <div className="p-20 text-center text-slate-500 font-bold uppercase tracking-widest opacity-30">
                                <span className="material-symbols-outlined text-6xl mb-4">info</span>
                                <p>Nenhum saldo pendente para reprogramação encontrado.</p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full text-left">
                                    <thead className="bg-[#0f172a] text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                        <tr>
                                            ...
                                            <th className="px-8 py-4 text-right">Saldo Atual (Reprogramável)</th>
                                        </tr>
                                    </thead>
                                            ...
                                            return (
                                                <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                                                    <td className="px-8 py-4">
                                                        <span className="text-sm font-bold text-white">{school?.name || 'Escola não identificada'}</span>
                                                    </td>
                                                    <td className="px-8 py-4">
                                                        <span className="text-xs font-black text-slate-500 uppercase">{item.program_id ? (item.program_id.slice(0, 8)) : 'GERAL'}</span>
                                                    </td>
                                                    <td className="px-8 py-4 text-right">
                                                        <span className={`text-sm font-mono font-bold ${item.balance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                ...
                <div className="space-y-4">
                    <div className="flex justify-end">
                        <button
                            ...
                            className="text-xs text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 font-bold uppercase tracking-widest flex items-center gap-2 transition-colors"
                        >
                                    ...
                                    {editingUser.gee && (
                                        <div>
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                >
                                                    {(editingUser.assignedSchools || []).length === schoolsByGee.length ? 'Desmarcar Todas' : 'Selecionar Todas'}
                                                </button>
                                            </div>
