...
import { generateRelatorioGerencialHTML } from '../lib/documentTemplates';
...
const FinancialEntries: React.FC<{ user: User }> = ({ user }) => {
    ...
    const handlePrintReport = () => {
        ...
        const html = generateRelatorioGerencialHTML(entries, filters, user.name);
        ...
        <div className="flex flex-col gap-6">
            ...
            {showReprogrammedModal && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                    <div className="w-full max-w-4xl bg-[#0f172a] border border-blue-500/20 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95">
                        ...
                        <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="bg-white/[0.02] p-6 rounded-2xl border border-white/5 flex flex-col gap-4 text-xs">
                                ...
                                <select value={newReprogrammed.program_id} onChange={e => setNewReprogrammed({ ...newReprogrammed, program_id: e.target.value })} className="bg-[#1e293b] rounded-lg h-10 px-3 text-white border-none outline-none">
                                    <option value="">Selecione...</option>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <select value={newReprogrammed.nature} onChange={e => setNewReprogrammed({ ...newReprogrammed, nature: e.target.value as TransactionNature })} className="bg-[#1e293b] rounded-lg h-10 px-2 text-white border-none outline-none">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <select value={newReprogrammed.period} onChange={e => setNewReprogrammed({ ...newReprogrammed, period: e.target.value })} className="bg-[#1e293b] rounded-lg h-10 px-2 text-white border-none outline-none">
                                            <option value="">...</option>
                                ...
                                <input type="number" step="0.01" value={newReprogrammed.value} onChange={e => setNewReprogrammed({ ...newReprogrammed, value: e.target.value })} className="bg-[#1e293b] rounded-lg h-10 px-3 text-white border-none outline-none" />
                                <button onClick={handleSaveReprogrammed} disabled={isSavingReprogrammed} className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 rounded-xl transition-all">Salvar Saldo</button>
                            </div>
                            <div className="lg:col-span-2 overflow-y-auto max-h-[400px] flex flex-col gap-2">
                                {reprogrammedBalances.map(rb => (
                                    <div key={rb.id} className="bg-white/5 p-4 rounded-xl flex justify-between items-center group">
                                        <div className="flex flex-col">
                                            <span className="text-white font-bold text-sm">{rb.period} - {rb.programs?.name}</span>
                                            <span className="text-[10px] text-slate-500">{rb.schools?.name}</span>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <span className="text-lg font-black text-white font-mono">{rb.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                                            <button onClick={() => handleDeleteReprogrammed(rb.id)} className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><span className="material-symbols-outlined">delete</span></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
